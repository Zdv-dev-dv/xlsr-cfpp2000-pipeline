...
1. **Data preparation** (`0_data_prep` section): the source recording is downloaded directly from the CFPP2000 public archive, resampled to 16 kHz mono, and split into consecutive 30-second non-overlapping segments saved to `data/segments/`.
...
**Note:** `data/segments/` and `output/` are listed in `.gitignore`. They are not versioned because the segments are derived from a public source and can be reproduced exactly by running cell 0. The expected output CSV (`transcriptions_xlsr.csv`) is also not committed; it is an artefact of running the notebook.
...
**Segmentation:** 50 non-overlapping 30-second segments extracted starting at t=0  
